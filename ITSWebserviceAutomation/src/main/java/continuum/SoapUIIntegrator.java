		...
		try {
		   ...
		   // retrieve all test suite from project
		    ...
		    for (TestSuite ts : testSuiteList) {
		    	 Reporter.log("****Running Test suite " + ts.getName() + "********");
		         ...
		         testCase = new String[testCaseList.size()][1];                                 
		          // Iterate all TestCases of the particular TestSuite
		           for (int i=0;i<testCaseList.size();i++)
		           {
		        	   Reporter.log("****Running Test case " + testCaseList.get(i).getName() + "********");
		        	   System.out.println("****Running Test case " + testCaseList.get(i).getName() + "********");
		        	   testCase[i][0]=	 testCaseList.get(i).getName() ;              
